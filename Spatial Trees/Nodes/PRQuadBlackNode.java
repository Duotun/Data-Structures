...
public class PRQuadBlackNode extends PRQuadNode {
    ...
    private ArrayList<KDPoint> points;
    ...
    public PRQuadBlackNode(KDPoint centroid, int k, int bucketingParam, Collection<KDPoint> list){
        this(centroid, k, bucketingParam); // Call to the current class' other constructor, which takes care of the base class' initialization itself.
        points = new ArrayList<KDPoint>();
        for (KDPoint point: list) points.add(point);
    }
    ...
    public PRQuadNode insert(KDPoint p, int k) {
        if (this.count() < this.bucketingParam) {
        	...
        	PRQuadGrayNode temp = new PRQuadGrayNode(this.centroid, k, this.bucketingParam);
        	for (KDPoint point: points) temp = (PRQuadGrayNode) temp.insert(point, k);
        	temp = (PRQuadGrayNode) temp.insert(p, k);
        	return temp;
        }
    }
    ...
    public PRQuadNode delete(KDPoint p) {
        if (!points.contains(p)) return this;
    ...
    public Collection<KDPoint> getPoints(){
        Collection<KDPoint> list = new ArrayList<KDPoint>();
        
        for (KDPoint point: points) list.add(point);
